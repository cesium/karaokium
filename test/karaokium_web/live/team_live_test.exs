defmodule KaraokiumWeb.TeamLiveTest do
  ...
  describe "Index" do
    setup [:register_and_log_in_admin_user, :create_team]

    test "lists all teams", %{conn: conn, team: team} do
      {:ok, _index_live, html} = live(conn, Routes.admin_team_index_path(conn, :index))
    ...
    test "saves new team", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, Routes.admin_team_index_path(conn, :index))

      assert index_live |> element("a", "New Team") |> render_click() =~
               "New Team"

      assert_patch(index_live, Routes.admin_team_index_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.admin_team_index_path(conn, :index))
    ...
    test "updates team in listing", %{conn: conn, team: team} do
      ...
      assert_patch(index_live, Routes.admin_team_index_path(conn, :edit, team))
  ...
  describe "Show" do
    ...
    test "displays team", %{conn: conn, team: team} do
      {:ok, _show_live, html} = live(conn, Routes.admin_team_show_path(conn, :show, team))
    ...
    test "updates team within modal", %{conn: conn, team: team} do
      {:ok, show_live, _html} = live(conn, Routes.admin_team_show_path(conn, :show, team))

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Team"

      assert_patch(show_live, Routes.admin_team_show_path(conn, :edit, team))
      ...
      {:ok, _, html} =
        ...
        |> form("#team-form", team: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.admin_team_show_path(conn, :show, team))
